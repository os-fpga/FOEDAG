...
# make IPA=1 enables Interactive Path Analysis Tool
...
run-cmake-release:
	cmake -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$(PREFIX) -DCMAKE_RULE_MESSAGES=$(RULE_MESSAGES) -DPRODUCTION_BUILD=$(PRODUCTION_BUILD) -DSTICK_RELEASE_VERSION=$(STICK_RELEASE_VERSION) -DMONACO_EDITOR=$(MONACO_EDITOR) -DIPA=$(IPA) $(ADDITIONAL_CMAKE_OPTIONS) -S . -B build

run-cmake-release_no_tcmalloc:
	cmake -DNO_TCMALLOC=On -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$(PREFIX) -DCMAKE_RULE_MESSAGES=$(RULE_MESSAGES) $(ADDITIONAL_CMAKE_OPTIONS) -DSTICK_RELEASE_VERSION=$(STICK_RELEASE_VERSION) -DMONACO_EDITOR=$(MONACO_EDITOR) -DIPA=$(IPA) -S . -B build

run-cmake-debug:
	cmake -DCMAKE_BUILD_TYPE=Debug -DCMAKE_INSTALL_PREFIX=$(PREFIX) -DCMAKE_RULE_MESSAGES=$(RULE_MESSAGES) -DPRODUCTION_BUILD=$(PRODUCTION_BUILD) -DSTICK_RELEASE_VERSION=$(STICK_RELEASE_VERSION) -DMONACO_EDITOR=$(MONACO_EDITOR) -DIPA=$(IPA) -DNO_TCMALLOC=On $(ADDITIONAL_CMAKE_OPTIONS) -S . -B dbuild

run-cmake-coverage:
	cmake -DCMAKE_BUILD_TYPE=Debug -DCMAKE_INSTALL_PREFIX=$(PREFIX) -DCMAKE_RULE_MESSAGES=$(RULE_MESSAGES) -DMY_CXX_WARNING_FLAGS="--coverage" $(ADDITIONAL_CMAKE_OPTIONS) -DSTICK_RELEASE_VERSION=$(STICK_RELEASE_VERSION) -DMONACO_EDITOR=$(MONACO_EDITOR) -DIPA=$(IPA) -S . -B coverage-build
...
valgrind_args = --log-file=valgrind_gui.log --gen-suppressions=all --suppressions=valgrind.supp
test/valgrind: run-cmake-debug
	...
	$(XVFB) valgrind --tool=memcheck $(valgrind_args) dbuild/bin/foedag --replay tests/TestGui/gui_start_stop.tcl;
	grep "ERROR SUMMARY: 0" valgrind_gui.log 
	$(XVFB) valgrind --tool=memcheck $(valgrind_args) dbuild/bin/newproject --replay tests/TestGui/gui_new_project.tcl
	grep "ERROR SUMMARY: 0" valgrind_gui.log
	...
	grep "ERROR SUMMARY: 0" valgrind_gui.log
	$(XVFB) valgrind --tool=memcheck $(valgrind_args) dbuild/bin/ipconfigurator --replay tests/TestGui/gui_ipconfigurator.tcl
	grep "ERROR SUMMARY: 0" valgrind_gui.log
...
test_install:
	cmake -DCMAKE_BUILD_TYPE=Release -DINSTALL_DIR=$(PREFIX) -DMONACO_EDITOR=$(MONACO_EDITOR) -DIPA=$(IPA) -S tests/TestInstall -B tests/TestInstall/build
