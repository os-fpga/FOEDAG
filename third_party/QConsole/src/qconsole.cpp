...
QSize PopupListWidget::sizeHint() const {
  ...
  for (int i = 0; i < this->count(); ++i) {
    ...
    if (itemSizeHint.width() > width) width = itemSizeHint.width();
...
// Clear the console
void QConsole::clear() { QTextEdit::clear(); }
...
bool QConsole::handleBackspaceKeyPress() {
  ...
  if (blk == promptParagraph && col == promptLength) return true;
  return false;
}
...
void QConsole::setHome(bool select) {
  ...
  cursor.movePosition(QTextCursor::StartOfBlock, select
  ...
  */
  ...
  if ((e->modifiers() & Qt::ControlModifier) && (e->key() == Qt::Key_C)) {
    if (isSelectionInEditionZone()) {
      ...
      //(Thierry Belair:)I humbly suggest that ctrl+C copies the text, as is
      //expected, and indicated in the contextual menu
      copy();
      return;
    }
  }
  if ((e->modifiers() & Qt::ControlModifier) && (e->key() == Qt::Key_R)) {
    ...
    switch (e->key()) {
      ...
      case Qt::Key_Backspace:
        if (handleBackspaceKeyPress() || !isSelectionInEditionZone()) return;
        break;
...
bool QConsole::isCommandComplete(const QString &) { return true; }
...
bool QConsole::isInEditionZone() {
  ...
  return (para > promptParagraph) ||
         ((para == promptParagraph) && (index >= promptLength));
}

// Tests whether position (in parameter) is in the edition zone or not (after
// the prompt or in the next lines (in case of multi-line mode)
...
int QConsole::saveScript(const QString &fileName) {
  QFile f(fileName);
  if (!f.open(WRITE_ONLY)) return -1;
  QTextStream ts(&f);
  for (QStringList::Iterator it = recordedScript.begin();
       it != recordedScript.end(); ++it)
    ts << *it << "\n";
  f.close();
  return 0;
}
