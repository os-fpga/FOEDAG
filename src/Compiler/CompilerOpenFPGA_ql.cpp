...
bool CompilerOpenFPGA_ql::Synthesize() {
  ...
  std::string settings_json_filename = m_design->Name() + ".json";
  ...
  for (QJsonValue setting : settingsSynthArray) {
    ...
    //   qDebug() << settingObj["id"].toString() << settingObj["default"].toString() << "\n";
      ...
      if(settingObj["id"].toString() == "verilog" && settingObj["default"].toString() == "checked") {
        yosys_options += " -verilog " + std::string(m_design->Name() + "_post_synth.v");
      }
      ...
      if(settingObj["id"].toString() == "edif" && settingObj["default"].toString() == "checked") {
        yosys_options += " -edif " + std::string(m_design->Name() + ".edif");
      }
    }
  }
...
//   QJsonArray settingsPOWER = currentSettings->getNested("Settings.Tasks.POWER", ".").toArray();
  ...
  std::filesystem::path architectureFilePath = std::filesystem::path(GetSession()->Context()->DataPath() / family / foundry / node / std::string("vpr.xml"));
  ...
  std::string command =
      ...
      architectureFilePath.string() + std::string(" ") +
      std::string(netlistFile) +
      // TODO: sdc file needs to be taken and preprocessed before use?
      //std::string(" --sdc_file ") + std::string(m_design->Name() + "_openfpga.sdc") +
      vpr_options;
  return command;
}
