...
bool CompilerOpenFPGA_ql::Analyze() {
  qInfo() << "~~~ CompilerOpenFPGA::Analyze";
  ...
  auto guard = sg::make_scope_guard([this] {
    ...
    //upstream // std::filesystem::path logPath = FilePath(Action::Analyze, ANALYSIS_LOG);
    std::filesystem::path projectPath(ProjManager()->projectPath());
    std::filesystem::path logPath = projectPath / ANALYSIS_LOG;
    LogUtils::AddHeaderToLog(logPath);
  });

  //upstream// auto printTopModules = [this](const std::filesystem::path& filePath,
  //upstream //                               std::ostream* out) {
  //upstream //   // Check for "topModule" in a given json filePath
  //upstream //   // Assumed json format is [ { "topModule" : "some_value"} ]
  //upstream //   if (out) {
  //upstream //     auto topModules = TopModules(filePath);
  //upstream //     if (!topModules.empty()) {
  //upstream //       (*out) << "Top Modules: " << StringUtils::join(topModules, ", ")
  //upstream //              << std::endl;
  //upstream //     }
  //upstream //   }
  //upstream // };
  ...
  if (!DesignChangedForAnalysis(analysisScript, script_path, output_path)) {
    ...
    //upstream //printTopModules(output_path, &tempOut);
    Message(tempOut.str());
    return true;
  }
  ...
  m_analyzeExecutablePath = m_yosysExecutablePath; // ~~~ new
  qInfo() << "~~~ m_yosysExecutablePath=" << m_yosysExecutablePath.string().c_str();
  QByteArray value = qgetenv("PATH");
  if (value.isEmpty()) {
      qDebug() << "Environment variable PATH is not set.";
  } else {
      qDebug() << "PATH =" << value;
  }
  if (GetParserType() == ParserType::Default ||
