...
bool CompilerOpenFPGA::Analyze() {
  ...
  if (raptor_log.good()) {
    ...
    if (buf.find("VERI-1063") != std::string::npos) {
      std::string modules = "";
      int pos = 0; 
      while (buf.find("instantiating unknown module ", pos) != std::string::npos) {
           pos = buf.find("instantiating unknown module ", pos) + 29; //the searched line size
           std::string tmp = " ";
           while (buf[pos] != ' ') {
                tmp += buf[pos];
                ++pos;
            }
            if (modules.find(tmp) == std::string::npos)
              modules += tmp;
        }
