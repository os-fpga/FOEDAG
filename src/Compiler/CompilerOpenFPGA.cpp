...
std::filesystem::path CompilerOpenFPGA::copyLog(
  ...
  if (projManager) {
    std::filesystem::path src = projManager->ProjectFile(srcFileName);
    if (FileUtils::FileExists(src)) {
      dest = projManager->ProjectFile(destFileName);
...
bool CompilerOpenFPGA::Analyze() {
  ...
  auto guard = sg::make_scope_guard([this] {
    ...
    std::filesystem::path logPath = ProjManager()->ProjectFile(ANALYSIS_LOG);
  ...
  auto script_path = ProjManager()->ProjectFile(ProjManager()->projectName() +
                                                "_analyzer.cmd");
  std::filesystem::path output_path =
      ProjManager()->ProjectFile("port_info.json");
  ...
  std::string command;
  int status = 0;
  std::filesystem::path analyse_path = ProjManager()->ProjectFile(ANALYSIS_LOG);
  if (m_useVerific) {
...
bool CompilerOpenFPGA::Synthesize() {
  ...
  std::filesystem::remove(ProjManager()->ProjectFile(
      ProjManager()->projectName() + "_post_synth.v"));
  // Create Yosys command and execute
  script_path = ProjManager()->ProjectFile(script_path).string();
...
std::string CompilerOpenFPGA::BaseStaScript(std::string libFileName,
  ...
  const auto openStaFile =
      ProjManager()->ProjectFile(ProjManager()->projectName() + "_opensta.tcl");
  FileUtils::WriteToFile(openStaFile, script);
  return openStaFile.string();
}
...
bool CompilerOpenFPGA::Packing() {
  ...
  Message("##################################################");
  const auto sdcOut = ProjManager()->ProjectFile(ProjManager()->projectName() +
                                                 "_openfpga.sdc");
  ...
  auto file =
      ProjManager()->ProjectFile(ProjManager()->projectName() + "_pack.cmd");
  FileUtils::WriteToFile(file, command);

  if (FileUtils::IsUptoDate(GetNetlistPath(), ProjManager()->ProjectFile(
  ...
  if (status) {
    ...
    if (PackOpt() == PackingOpt::Debug) {
      std::string command = BaseVprCommand() + " --pack";
      auto file = ProjManager()->ProjectFile(ProjManager()->projectName() +
                                             "_pack.cmd");
      FileUtils::WriteToFile(file, command);
...
bool CompilerOpenFPGA::Placement() {
  ...
  const auto pcfOut = ProjManager()->ProjectFile(ProjManager()->projectName() +
                                                 "_openfpga.pcf");
  ...
  if ((previousConstraints == newConstraints) &&
      FileUtils::IsUptoDate(
          ProjManager()->ProjectFile(ProjManager()->projectName() +
                                     "_post_synth.net"),
          ProjManager()->ProjectFile(ProjManager()->projectName() +
                                     "_post_synth.place"))) {
    m_state = State::Placed;
  ...
  if ((PinAssignOpts() != PinAssignOpt::Free) &&
    ...
    if (!set_clks.empty() && repackConstraint) {
      const auto repack_out = ProjManager()->ProjectFile(
  ...
  auto file =
      ProjManager()->ProjectFile(ProjManager()->projectName() + "_place.cmd");
  FileUtils::WriteToFile(file, command);
  int status = ExecuteAndMonitorSystemCommand(command);
  if (status) {
    ErrorMessage("Design " + ProjManager()->projectName() +
...
bool CompilerOpenFPGA::Route() {
  ...
  FileUtils::WriteToFile(
      ProjManager()->ProjectFile(ProjManager()->projectName() + "_route.cmd"),
      command);
  int status = ExecuteAndMonitorSystemCommand(command);
  if (status) {
...
bool CompilerOpenFPGA::TimingAnalysis() {
  ...
  if (TimingAnalysisEngineOpt() == STAEngineOpt::Opensta) {
    ...
    auto file =
        ProjManager()->ProjectFile(ProjManager()->projectName() + "_sta.cmd");
    FileUtils::WriteToFile(file, command);
    int status = ExecuteAndMonitorSystemCommand(command);
    if (status) {
      ErrorMessage("Design " + ProjManager()->projectName() +
    ...
    auto netlistFileName = ProjManager()->ProjectFile(
        ProjManager()->projectName() + "_post_synthesis.v");
    auto sdfFileName = ProjManager()->ProjectFile(ProjManager()->projectName() +
                                                  "_post_synthesis.sdf");
    ...
    if (std::filesystem::is_regular_file(libFileName) &&
      ...
      taCommand = BaseStaCommand() + " " +
                  BaseStaScript(libFileName, netlistFileName.string(),
                                sdfFileName.string(), sdcFileName);
      auto file =
          ProjManager()->ProjectFile(ProjManager()->projectName() + "_sta.cmd");
...
std::string CompilerOpenFPGA::FinishOpenFPGAScript(const std::string& script) {
  ...
  std::string repack_constraints =
      ProjManager()->projectName() + "_repack_constraints.xml";
  const bool fpga_repack =
      FileUtils::FileExists(ProjManager()->ProjectFile(repack_constraints));
  if (!fpga_repack) {
...
bool CompilerOpenFPGA::GenerateBitstream() {
  ...
  if (BitsOpt() == BitstreamOpt::EnableSimulation) {
    std::filesystem::path bit_path = ProjManager()->ProjectFile("BIT_SIM");
  ...
  if (FileUtils::IsUptoDate(
          ProjManager()->ProjectFile(ProjManager()->projectName() +
                                     "_post_synth.route"),
          ProjManager()->ProjectFile("fabric_bitstream.bit"))) {
  ...
  std::filesystem::remove(ProjManager()->ProjectFile("fabric_bitstream.bit"));
  std::filesystem::remove(
      ProjManager()->ProjectFile("fabric_independent_bitstream.xml"));
