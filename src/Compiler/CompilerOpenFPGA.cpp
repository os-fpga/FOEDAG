...
bool CompilerOpenFPGA::Analyze() {
  ...
  auto parser = GetParserType();
  auto guard = sg::make_scope_guard([this, parser] {
  ...
  if (status) {
    if (GetParserType() == ParserType::Default) {
      // save default parser cmd
      FileUtils::WriteToFile(
          script_path.parent_path() /
              (ProjManager()->projectName() + "_analyzer_default.cmd"),
...
bool CompilerOpenFPGA::Synthesize() {
  ...
  if (status) {
    if (GetParserType() == ParserType::Default) {
      FileUtils::WriteToFile(ProjManager()->projectName() + "_default.ys",
                             yosysScript, false);
      // If Default Yosys parser fails, attempt with the Surelog parser
      SetParserType(ParserType::Surelog);
