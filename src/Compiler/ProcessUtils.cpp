...
void ProcessUtils::HandleOutput(const QString &output) {
  ...
  // TODO: Windows
#else
  auto index1 = output.indexOf("VmPeak");
  auto index2 = output.indexOf("VmSize");
  auto t = output.mid(index1, index2 - index1);
  auto splitted = t.split(" ",
  ...
  if (splitted.count() > 1) {
    m_max_utiliation = std::max(m_max_utiliation, splitted.at(1).toUInt());
  }
#endif
}
...
QStringList ProcessUtils::Arguments(int processId) const {
  ...
  return QStringList();
#else
  return {QString("/proc/%1/status").arg(processId)};
#endif
}
