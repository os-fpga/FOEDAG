...
bool Compiler::RegisterCommands(TclInterpreter* interp, bool batchMode) {
  ...
  auto create_design = [](void* clientData, Tcl_Interp* interp, int argc,
    ...
    if (compiler->m_tclCmdIntegration) {
      ...
      bool ok{true};
      if (compiler->m_tclCmdIntegration->getActiveDesign().isEmpty()) {
        ok = compiler->m_tclCmdIntegration->TclCreateProject(argc, argv, out);
      } else {
        ok = compiler->m_tclCmdIntegration->TclCreateFileSet(name.c_str(), out);
      }
      if (!out.str().empty()) {
        Tcl_AppendResult(interp, out.str().c_str(), nullptr);
      }
      if (!ok) return TCL_ERROR;
      compiler->Message(std::string("Created design source: ") + name +
                        std::string("\n"));
    }
...
bool Compiler::CreateDesign() {
  if (m_design == nullptr) {
    int result = Tcl_Eval(m_interp->getInterp(), "create_design noname");
    if (result != TCL_OK) return false;
  }
  if (m_design == nullptr) return false;
  return true;
}
