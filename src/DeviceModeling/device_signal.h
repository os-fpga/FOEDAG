...
public:
    ...
    void initialize_vec(unsigned size)
    {
        for (unsigned int i = 0; i < size; ++i)
        {
            ...
            oss << name_ << "__" << i << "__";
            net_vector_.push_back(std::make_shared<device_net>(oss.str(), shared_from_this()));
        }
    }
    /**
     ...
     */
    const std::vector<std::shared_ptr<device_net>> &get_net_vector() const { return net_vector_; }
