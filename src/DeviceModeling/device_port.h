...
public:
     ...
     */
    device_port(const std::string &name,
                bool is_in = false,
                device_signal *signal_ptr = nullptr,
                device_block *block_ptr = nullptr)
        : name_(name), is_in_(is_in), signal_ptr_(signal_ptr), enclosing_block_ptr_(block_ptr)
    {
    }
     ...
     */
    device_port()
        : name_("__default_port_name__"), is_in_(false), signal_ptr_(nullptr), enclosing_block_ptr_(nullptr)
    {
    }
    ...
    bool equal(const device_port &other) const
    {
        if (this->name_ != other.name_)
            return false;
        if (this->is_in_ != other.is_in_)
            return false;
        // Compare pointers. Depending on your use case, you might want to compare the objects they're pointing to.
        if (this->signal_ptr_ != other.signal_ptr_)
            return false;
        if (this->enclosing_block_ptr_ != other.enclosing_block_ptr_)
            return false;
        return true;
    }
    ...
    friend std::ostream &operator<<(std::ostream &os, const device_port &port)
    {
        os << "Port Name: " << port.name_
           << ", Direction: " << (port.is_in_ ? "Input" : "Output")
           << ", Signal: ";
...
private:
    ...
    device_signal *signal_ptr_;         ///< A pointer to the signal driven by an input port or driving an output port
    device_block *enclosing_block_ptr_; ///< A pointer to the block for which this is a port
};
