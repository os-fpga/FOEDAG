...
/**
 * @brief A class that represents a named parameter with a specific type and value.
...
public:
     ...
     * @throws runtime_error If the value doesn't respect the type's bounds or enumeration.
     */
    Parameter(const std::string &name, const T &value, const std::shared_ptr<ParameterType<T>> &type)
        : name_(name), type_ptr_(type)
    {
        ...
        {
            throw std::runtime_error("Value doesn't respect the bounds or enumeration");
        }
    }
     ...
     * @throws runtime_error If the value doesn't respect the type's bounds or enumeration.
     */
    Parameter(const std::string &name, const std::shared_ptr<ParameterType<T>> &type)
        : name_(name), type_ptr_(type)
    {
     ...
     */
    std::shared_ptr<ParameterType<T>> get_type() const
    {
        return type_ptr_;
    }
     ...
     */
    bool operator==(const Parameter<T> &rhs) const
    {
        return (this == &rhs);
    }
