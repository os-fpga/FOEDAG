...
public:
     ...
     * @throws std::runtime_error If the size is not set or the value does not fit in the size.
     */
    void set_enum_value(std::string key, unsigned int value)
    {
        if (!has_size())
        {
            throw std::runtime_error("Please set the size of a parameter before setting the enum values");
        }
        unsigned mx = 1 << get_size();
        if (value >= mx)
        {
            throw std::runtime_error("The value " + std::to_string(value) + " can not fit within " + std::to_string(get_size()) + " bits");
        }
     ...
     * @throws std::runtime_error If the value does not fit in the size, or is outside the bounds.
     */
    void set_default_value(T value)
    {
        if constexpr (std::is_same<T, int>::value)
        {
            if (has_size())
            {
                ...
                if (value < 0 || value >= mx)
                {
                    throw std::runtime_error("Invalid default value as the parameter size is set to " + std::to_string(get_size()) + " bits");
                }
            }
        }
        if constexpr (! std::is_same<T, std::string>::value)
        {
            ...
            {
                throw std::runtime_error("Invalid default value as the parameter lower bound is set to " + std::to_string(get_lower_bound()));
            }
            if (has_upper_bound() && value > get_upper_bound())
            {
                throw std::runtime_error("Invalid default value as the parameter upper bound is set to " + std::to_string(get_upper_bound()));
            }
        }
     ...
     * For int and double type parameters, the value is valid if it fits within the size (for int type),
     * and is within the set lower and upper bounds (if they are set).
     *
