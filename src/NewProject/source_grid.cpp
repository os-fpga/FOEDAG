...
/*    tmenu->setStyleSheet("QMenu{padding:6px 16px; spacing:5;font-size:20px;}\
...
//    m_toolBar->addAction(m_actDown);
    ...
    connect(m_selectmodel,&QItemSelectionModel::selectionChanged,this,&sourceGrid::slot_selectionChanged);
...
QList<filedata> sourceGrid::getgriddata()
{
    return m_fdatalist;
}
...
void sourceGrid::slot_addfiles()
{
    ...
    QStringList fileNames = QFileDialog::getOpenFileNames(this,tr("Select File"),"", fileformat);
...
{
    QString pathName = QFileDialog::getExistingDirectory(this, tr("Select Directory"),"",
                                                         QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks);
    if(""==pathName)return;
    QString folder = pathName.right(pathName.size() - (pathName.lastIndexOf("/")+1));
...
//    connect(createdlg,&createfiledialog::sig_updategrid,this, &sourceGrid::slot_addgridrow);
//    createdlg->exec();
//    disconnect(createdlg,&createfiledialog::sig_updategrid,this, &sourceGrid::slot_addgridrow);
}
...
void sourceGrid::slot_deletegriditem()
{
    ...
    QStandardItemModel *model = qobject_cast<QStandardItemModel*>(m_grid->model());
    if (model == nullptr) {
        return;
    }
