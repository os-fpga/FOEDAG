...
addSourceForm::addSourceForm(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::addSourceForm)
{
    ui->setupUi(this);
    ui->m_labelTitle->setText(tr("Add Sources"));
    ui->m_labelDetail->setText(tr("Specify design files,or directories containing those files,to add to your project."
                                  "Create a new source file on disk and add it to your project.You can also add and create source later."));
...
{
    return ui->m_ckkBoxCopy->checkState()==Qt::CheckState::Checked ? true : false;
}
