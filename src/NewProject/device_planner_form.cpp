    ...
    ui(new Ui::devicePlannerForm)
{
    ...
    ui->m_labelDetail->setText(tr("Select the series and device you want to target for compilation."));
    ...
    m_grid->horizontalHeader()->setStretchLastSection(true);     //Last column adaptive width
    m_grid->setEditTriggers(QTableView::NoEditTriggers);
    m_grid->setSelectionBehavior(QTableView::SelectRows);
    m_grid->setSelectionMode(QTableView::SingleSelection);       //Single line selection
    m_grid->setAlternatingRowColors(true);                       //Color separation between lines

    m_grid->setStyleSheet("QTableView {border: 1px solid rgb(230,230,230);}\
    ...
    connect(ui->m_comboBoxSeries,&QComboBox::currentTextChanged,this,&devicePlannerForm::onSeriestextChanged);
    connect(ui->m_comboBoxPackage,&QComboBox::currentTextChanged,this,&devicePlannerForm::onPackagetextChanged);
}
...
void devicePlannerForm::getdeviceinfo(QString &servies, QString &device, QString &package)
{
...
void devicePlannerForm::initformdata_fpga(QString dir)
{
    ...
    {
        QMessageBox::warning(this,tr("error"),tr("device_fpga.xml read error!"),QMessageBox::Ok);
        return;
    }
    ...
    {
        file.close();
        QMessageBox::warning(this,tr("error"),tr("device_fpga.xml setContent failed!"),QMessageBox::Ok);
        return;
    }
    file.close();
...
void devicePlannerForm::initformdata_efpga(QString dir)
{
    ...
    if(!file.open(QFile::ReadOnly))
    {
        QMessageBox::warning(this,tr("error"),tr("device.xml read error!"),QMessageBox::Ok);
        return;
    }
    ...
    {
        file.close();
        QMessageBox::warning(this,tr("error"),tr("device.xml setContent failed!"),QMessageBox::Ok);
        return;
    }
    file.close();
    ...
    while(!node.isNull())
    {
        if(node.isElement())
        {
            ...
            //qDebug()<<e.tagName()<<" "<<e.attribute("Name") << e.attribute("Type") << e.attribute("Series");
...
{
    disconnect(ui->m_comboBoxSeries,&QComboBox::currentTextChanged,this,&devicePlannerForm::onSeriestextChanged);
    disconnect(ui->m_comboBoxPackage,&QComboBox::currentTextChanged,this,&devicePlannerForm::onPackagetextChanged);
