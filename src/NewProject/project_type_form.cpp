    ...
    ui(new Ui::projectTypeForm)
{
    ...
    ui->m_labelRTL->setText(tr("You will be able to add sources,create block designs in IP integrator,generate IP, "
                               "run RTL analysis,synthesis,implementation,design planning and analysis."));
    ui->m_labelPost->setText(tr("You will be able to add sources, view device resources,run design analysis,planning and implementation."));
...
bool projectTypeForm::isaddsource()
{
    return true;
}

void projectTypeForm::on_m_radioBtnRTL_clicked()
{
    updatestate();
}

void projectTypeForm::on_m_radioBtnPost_clicked()
{
    updatestate();
}
