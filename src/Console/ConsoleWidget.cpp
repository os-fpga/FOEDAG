...
QStringList ConsoleWidget::suggestCommand(const QString &cmd, QString &prefix) {
  ...
  //    prefix = cmd.left(i+1);
  //  }
  //  int res = Tcl_Eval( interp, qPrintable("info commands [join {" +
  //  commandToComplete + "*}]") ); if (!res)
  //  {
...
//      if (m_lastPos < textCursor().position())
