...
/**
 * @brief Basic logger for recording information to either a separate file or
 * the screen.
 *
 * This logger supports logging data to a designated file or displaying it on
 * the screen. Additionally, it includes a feature to truncate log files if they
 * exceed a specified size limit.
 */
class SimpleLogger {
 ...
 public:
  ...
  void log(Args... args) {
    ...
    if (m_file.isOpen()) {
      ...
      qWarning() << "Could not write" << *out_str.string()
                 << "to log file, file is not opened" << m_filePath;
    }
 ...
 private:
  ...
  bool trimBeginningOfFile(const QString& fileName, qint64 nBytesToTrim) {
    ...
    if (!file.seek(nBytesToTrim)) {
      qDebug() << "Failed to seek to position" << nBytesToTrim << "in file"
               << fileName;
      file.close();
      return false;
    }
    ...
    if (!QFile::rename(tempFile.fileName(), fileName)) {
      qDebug() << "Failed to rename temporary file to original file name:"
               << fileName;
      return false;
    }
