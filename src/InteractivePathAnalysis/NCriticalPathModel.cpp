...
QModelIndex NCriticalPathModel::findPathElementIndex(
    const QModelIndex& pathIndex, const QString& elementData, int column) {
  if (pathIndex.isValid()) {
    NCriticalPathItem* pathItem =
        static_cast<NCriticalPathItem*>(pathIndex.internalPointer());
    if (pathItem) {
      if (pathItem->isPath()) {
...
void NCriticalPathModel::setupModelData(const std::vector<GroupPtr>& groups) {
  ...
  auto extractPathIndex = [](const QString& message) {
    ...
    if (match.hasMatch()) {
      ...
      if (ok) {
        return index - 1;  // -1 here is because the path index starts from 1,
                           // not from 0
      }
    }
    ...
    if (group->isPath()) {
      ...
      for (const auto& element : group->elements) {
        ...
        if (role == PATH) {
          ...
          pathItem = new NCriticalPathItem(data, val1, val2, type, id, pathId,
                                           isSelectable, m_rootItem);
          ...
          if (pathItem) {
            ...
            // we skip selection of index 0, as it's doesn't affect the render.
            // only when index 1 will be selected the line between node(0) and
            // node(1) will be rendered
            bool isSelectable = (id != 0);
            NCriticalPathItem* newItem = new NCriticalPathItem(
                data, val1, val2, type, id, pathId, isSelectable, pathItem);
      ...
      for (const auto& element : group->elements) {
        for (const Line& line : element->lines) {
          ...
          NCriticalPathItem* newItem = new NCriticalPathItem(
              data, val1, val2, type, id, pathId, isSelectable, m_rootItem);
...
std::tuple<QString, QString, QString> NCriticalPathModel::extractRow(
    QString l) const {
  l = l.trimmed();
