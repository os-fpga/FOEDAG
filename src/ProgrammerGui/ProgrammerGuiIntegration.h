...
class ProgrammerGuiIntegration : public QObject, public ProgrammerGuiInterface {
 ...
 public:
  ...
  void Flash(const Cable &cable, const Device &device) override;

  const std::pair<Cable, Device> &CurrentDevice() const;
  bool IsFlash() const;
  const std::vector<std::pair<Device, std::string>> &Files() const;
  std::string File(const Device &dev) const;
 ...
 private:
  ...
  std::vector<std::pair<Device, std::string>> m_files;
  bool m_flash{false};
};
