...
int main(int, char*[]) {
    ...
    inputFile.open("JSON_Files/GPIO_Yosis_Ver_Example/sdt_dev_zaid_rapidsilicon_example_soc_v5.json");
    ...
    *JSON to SDT cpp script written by ZaidTahir, for questions please email: zaid.butt.tahir@gmail.com or zaidt@bu.edu\n*/\n\n";
    ...
    // setting verbose flag to 0 and testing out printing the string returned by gen functions cx Raptor software doesnt allow cout directly

    gen_rootmetadata_node(outfile, rootmetadata_node_obj, verbose_flag_global);
    // buffer_gen_rootmetadata_node = gen_rootmetadata_node(outfile,rootmetadata_node_obj, verbose_flag_global);
    ...
    // buffer_gen_cpus_node = gen_cpus_node(outfile, cpus_node_obj, verbose_flag_global);
    ...
    // buffer_gen_cpus_cluster_node = gen_cpus_cluster_node(outfile, cpus_cluster_node_obj, verbose_flag_global);
    ...
    // buffer_gen_memory_node = gen_memory_node(outfile, memory_node_obj, verbose_flag_global);
    // cout << "gen_memory_node output = \n\n" << buffer_gen_memory_node << endl;


    gen_soc_node(outfile, soc_node_obj, verbose_flag_global);
    // buffer_gen_soc_node = gen_soc_node(outfile, soc_node_obj, verbose_flag_global);
