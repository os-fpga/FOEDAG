...
class SdtRootMetaDataNode {
    ...
    int object_has_been_populated; // must become 1 when class variables have been populated
    ...
    SdtRootMetaDataNode () {
        ...
        // if a string variable of our object has a empty string value, it means it wasn't present in the json (wasn't assigned a value)
...
// class for storing data of sdt interrupt controller meta data inside a cpu instantiations in the cpu sub-node
class SdtCpuInstSubNodeInterruptControllerData {
    ...
    SdtCpuInstSubNodeInterruptControllerData () {
        ...
        // if a string variable of our object has a empty string value, it means it wasn't present in the json (wasn't assigned a value)
            // except for value of int_cont_interrupt_controller_key which is empty in JSON, but if key is present we will
            // know to include it in SDT
        int_cont_phandle = ""; 
        int_cont_compatible = "";
...
class SdtCpusNode {
    ...
    // declare SdtCpuInstSubNode pointer to pointer for ptr to ptr array that would store pointer that would point to
    //  SdtCpuInstSubNode objects
        // malloc size to it depending upon "size_cpu_inst_array"
...
class SdtCpusClusterNode {
    ...
    // declare SdtCpuClusterInstSubNode pointer to pointer for ptr to ptr array that would store pointer that would point to
    //  SdtCpuClusterInstSubNode objects
        // malloc size to it depending upon "size_cpu_cluster_inst_array"
...
class SdtMemoryInstSubNode {
    // this is not a sub node actually.. each sub not for memory is an indenpendant node rather than being a subnode. 
    // So no need to have a subnode_name here cx we already gave the name to the memory node
...
class SdtMemoryNode {
    ...
    // declare SdtMemoryInstSubNode pointer to pointer for ptr to ptr array that would store pointer that would point to
...
class SdtSocInstSubNodeInterruptController {
    ...
    // reg addresses and reg sizes are in form of string arrays, but since we will have complete info in reg key in the json
    // thats why we arent using reg size and reg address for now, we'll have to use dynamic string array allocation for this like reg names
    // string interrupt_controller_reg_size; 
    // string interrupt_controller_reg_address;

    // will use dynamic string array allocation for this, ref "https://stackoverflow.com/questions/20207400/dynamically-allocated-string-array-then-change-its-value"
    // string * interrupt_controller_reg_names_string_array = NULL;
    ...
    int interrupt_controller_reg_names_string_array_size;
        // this variable is vip for knowing how many elements are in the reg_names string array
    ...
    SdtSocInstSubNodeInterruptController (string soc_subsystem_subnode_name) {
        ...
        interrupt_controller_key_value = "not empty";  // cx we need this to be empty in json after parsing it
...
class SdtSocInstSubNode {
    ...
    SdtSocInstSubNode (string soc_subsystem_type) {
        ...
        if (soc_subsystem_type == "interrupt-controller") {
            ...
            // SdtSocInstSubNodeInterruptController soc_interrupt_controller_object(soc_subsystem_type);
            // SdtSocInstSubNodeInterruptController soc_interrupt_controller_object_copy(soc_subsystem_type);
            // soc_interrupt_controller_object = &soc_interrupt_controller_object_copy;
            // soc_interrupt_controller_object = (SdtSocInstSubNodeInterruptController*)&soc_interrupt_controller_object_copy;

            // VVVIPPPP ... the above method did not work due to the reason written below:
                // https://stackoverflow.com/questions/26739197/return-with-ampersand
                // That depends on the lifetime of myStructObject. If it is a local variable it gets destroyed when you return from the function and results in undefined behavior.

            soc_interrupt_controller_object = new SdtSocInstSubNodeInterruptController(soc_subsystem_type);
            // cout << "inside SdtSocInstSubNode in the if cond when soc_subsystem_type = int cont \n" << endl;
...
class SdtSocNode {
    ...
    // declare SdtSocInstSubNode pointer to pointer for ptr to ptr array that would store pointer that would point to
    //  SdtSocInstSubNode objects
        // malloc size to it depending upon "size_soc_inst_array"
    ...
    SdtSocNode () {
        ...
        soc_size_cell = "";
        soc_compatible = "";
        soc_ranges_key_value = "not empty"; // since this needs to be empty after json is 
        soc_node_name = "soc";
    }
...
int get_soc_node(nlohmann::json data, SdtSocNode &sdt_soc_node_obj, int verbose=0);

// void get_memory_node(json data, SdtMemoryNode &sdt_memory_node_obj) {
int get_memory_node(nlohmann::json data, SdtMemoryNode &sdt_memory_node_obj, int verbose=0);

// void get_cpus_cluster(json data, SdtCpusClusterNode &sdt_cpus_cluster_node_obj) {
int get_cpus_cluster_node(nlohmann::json data, SdtCpusClusterNode &sdt_cpus_cluster_node_obj, int verbose=0);


// void get_rootmetadata_node (nlohmann::json data, SdtRootMetaDataNode &sdt_rootmetadata_node_obj) {
int get_rootmetadata_node (nlohmann::json data, SdtRootMetaDataNode &sdt_rootmetadata_node_obj, int verbose=0);


// void get_cpus (json data, SdtCpusNode &sdt_cpus_node_obj) {
int get_cpus_node (nlohmann::json data, SdtCpusNode &sdt_cpus_node_obj, int verbose=0);

string return_string_from_ofstream_file(ofstream &outfile, string file_path);

// passing &output by ref cx we wana change it
// string gen_rootmetadata_node(ofstream &outfile, SdtRootMetaDataNode sdt_rootmetadata_node_obj, int verbose=0) {
int gen_rootmetadata_node(ofstream &outfile, SdtRootMetaDataNode sdt_rootmetadata_node_obj, int verbose=0);

// string gen_cpus_cluster_node(ofstream &outfile, SdtCpusClusterNode sdt_cpus_cluster_node_obj, int verbose=0) {
int gen_cpus_cluster_node(ofstream &outfile, SdtCpusClusterNode sdt_cpus_cluster_node_obj, int verbose=0);

string gen_soc_subsystem_timer(stringstream &buffer_out, SdtSocInstSubNodeTimer * soc_timer_object, int verbose=0);

string gen_soc_subsystem_syscon(stringstream &buffer_out, SdtSocInstSubNodeSyscon * soc_syscon_object, int verbose=0);


string gen_soc_subsystem_gpio(stringstream &buffer_out, SdtSocInstSubNodeGpio * soc_gpio_object, int verbose=0);


string gen_soc_subsystem_uart(stringstream &buffer_out, SdtSocInstSubNodeUart * soc_uart_object, int verbose=0);


string gen_soc_subsystem_int_controller(stringstream &buffer_out, SdtSocInstSubNodeInterruptController * soc_interrupt_controller_object, int verbose=0);

// string gen_soc_node(ofstream &outfile, SdtSocNode sdt_soc_node_obj, int verbose=0) {
int gen_soc_node(ofstream &outfile, SdtSocNode sdt_soc_node_obj, int verbose=0);


// passing &output by ref cx we wana change it
// string gen_memory_node(ofstream &outfile, SdtMemoryNode sdt_memory_node_obj, int verbose=0) {
int gen_memory_node(ofstream &outfile, SdtMemoryNode sdt_memory_node_obj, int verbose=0);

// passing &output by ref cx we wana change it
// string gen_cpus_node(ofstream &outfile, SdtCpusNode sdt_cpus_node_obj, int verbose=0) {
int gen_cpus_node(ofstream &outfile, SdtCpusNode sdt_cpus_node_obj, int verbose=0);
