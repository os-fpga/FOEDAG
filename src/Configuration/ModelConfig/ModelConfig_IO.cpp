...
#define POST_MSG(type, space, ...) \
  { post_msg(type, space, CFG_print(__VA_ARGS__)); }
...
struct FCLK_GB_ROUTER {
  ...
  std::string route(std::string type, uint32_t bank, std::string pin,
    ...
    // clang-format off
    /*
      Each Bank has two FCLKs
        Each FCLK can generate fast_phase_clk_A and fast_phase_clk_B respectively 
          1. fast_phase_clk_A can clock gearbox 0 - 9 
          2. fast_phase_clk_B can clock gearbox 10 - 19
        Checking rules:
          1. You cant used more than two FCLKs per bank
          2. No two pins can use same fast_phase_clk_A or fast_phase_clk_B
              OR
             No two pins can use same FCLK
    */
    // clang-format on
    CFG_ASSERT(type == "HP" || type == "HVL" || type == "HVR");
    CFG_ASSERT(bank == 0 || bank == 1);
    CFG_ASSERT(fclk == 0 || fclk == 1);
    std::string msg = "";
    if (routers.find(type) == routers.end()) {
      routers[type] = {};
    }
    if (routers[type].find(bank) == routers[type].end()) {
      routers[type][bank] = {};
    }
    if (routers[type][bank].find(pin) == routers[type][bank].end()) {
      routers[type][bank][pin] = {};
    }
    bool need_to_add = false;
    if (std::find(routers[type][bank][pin].begin(),
                  routers[type][bank][pin].end(),
                  fclk) == routers[type][bank][pin].end()) {
      need_to_add = true;
    }
    if (need_to_add) {
      uint32_t count = 0;
      for (auto p : routers[type][bank]) {
        count += (uint32_t)(p.second.size());
        if (p.first != pin) {
          // This resource had not been used by other pin
          if (std::find(routers[type][bank][p.first].begin(),
                        routers[type][bank][p.first].end(),
                        fclk) != routers[type][bank][p.first].end()) {
            msg = CFG_print("FCLK resource in %s bank %d had been used by %s",
                            type.c_str(), bank, p.first.c_str());
          }
        }
      }
      if (count == 2) {
        // No FCLK resource
        std::string temp =
            CFG_print("All FCLK resource in %s bank %d had been used",
                      type.c_str(), bank);
        if (msg.size()) {
          msg = CFG_print("%s; %s", msg.c_str(), temp.c_str());
        } else {
          msg = temp;
        }
      }
  ...
  // [type][bank][pin] = list
  std::map<std::string,
           std::map<uint32_t, std::map<std::string, std::vector<uint32_t>>>>
      routers;
};
