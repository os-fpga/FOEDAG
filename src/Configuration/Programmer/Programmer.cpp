...
void programmer_entry(const CFGCommon_ARG* cmdarg) {
  ...
  if (cmdarg->compilerName == "dummy") {
    if (subCmd == "list_device") {
      ...
      CFG_POST_MSG("<test> ----- -------------------- ------------ ----------");
      CFG_POST_MSG("<test> Found  1 Gemini             0x1000AABB   5");
      CFG_POST_MSG("<test> Found  2 Gemini             0x2000CCDD   5");
      ...
      CFG_POST_MSG("<test> 1 UsbProgrammerCable_1_1");
      CFG_POST_MSG("<test> 2 UsbProgrammerCable_1_2");
      nonConstCmdArg->tclOutput = "UsbProgrammerCable1_1 UsbProgrammerCable1_2";
